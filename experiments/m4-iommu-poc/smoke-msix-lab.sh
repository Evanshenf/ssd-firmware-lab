...
lab_module_path=${3:-$kernel_module_dir/ssd_fwlab_msix_lab.ko}
test_stage=${4:-4}
iommu_module_name=ssd_fwlab_sw_iommu_poc
pci_module_name=ssd_fwlab_synth_pci_poc
lab_module_name=ssd_fwlab_msix_lab
lab_driver_name=ssd_fwlab_msix_lab
iommu_loaded=0
pci_loaded=0
lab_loaded=0
bdf=
...
case "$test_stage" in
	1|2|3|4) ;;
	*) echo "test stage must be 1..4" >&2; exit 2 ;;
esac

insmod "$lab_module_path" test_stage="$test_stage"
lab_loaded=1
...
lspci -D -s "$bdf" -nnvv | sed -n '1,80p'
...
if [ "$test_stage" -eq 4 ]; then
	...
	printf '%s\n' "$new_log" | grep -Fq \
		"M4-C stage $test_stage completed: PASS"
fi
if printf '%s\n' "$new_log" | grep -Eiq \
...
echo "M4-C isolated MSI-X lab stage $test_stage: PASS"

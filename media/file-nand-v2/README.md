...
## Generated records and persistent validation

The write path validates the request and reads/validates the current block
before constructing its private INTENT. It reuses the CRC just generated by
the encoder, without immediately decoding its own serialized bytes again.
The existing byte-fixture tests perform that round-trip check instead.
Recovery still fully decodes persisted INTENTs and matches their CRC to the
terminal record. Page/main/OOB integrity, NAND transition checks and all three
synchronization boundaries are unchanged. This optimization applies to both
builds; `FWLAB_MEDIA_EXCLUSIVE` remains only a host-file validation policy.

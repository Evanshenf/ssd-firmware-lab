...
"""Compare GCC and Clang C3.5c canonical lane bytes without hashing first."""
...
def main() -> int:
    ...
    try:
        ...
        print(f"C3.5c determinism: FAIL: {error}", file=sys.stderr)
        return 1
    hashes = " ".join(
        f"{mode}={hashlib.sha256(data).hexdigest()}"
        for mode, data in reference.items())
    print(f"C3.5c determinism: PASS (GCC == Clang exact bytes) {hashes}")
    return 0

...
tool_path=${2:-tools/vfio-cdev-v1/build/vfio_cdev_v1_c2_2}
	...
	if [ "$loaded" -eq 1 ] || [ "$module_present" -eq 1 ]; then
		if [ "$module_present" -eq 1 ]; then
			if rmmod "$module_name"; then
				...
				# Another cleanup path won the race; do not rmmod twice.
				loaded=0
			fi
		else
			# Covers a signal after successful rmmod but before loaded=0.
			loaded=0
		fi
	fi
	count=0
...
if [ ! -f "$module_path" ] || [ ! -x "$tool_path" ]; then
	echo "module or C2.2 tool missing" >&2
	exit 2
fi
if ! command -v timeout >/dev/null 2>&1; then
	echo "GNU timeout is required for the privileged C2.2 gate" >&2
	exit 2
fi
...
if ! "$tool_path" --selftest-layout; then
	echo "C2.2 pure region-layout selftest failed" >&2
	exit 1
fi
echo "phase=layout-selftest-passed"
...
set +e
timeout -k 5s 60s "$tool_path" "/dev/vfio/devices/$vfio_name"
tool_rc=$?
set -e
if [ "$tool_rc" -ne 0 ]; then
	echo "C2.2 userspace tool failed or timed out: rc=$tool_rc" >&2
	exit 1
fi
echo "phase=c2.2-userspace-passed"
...
echo "C2.2 V1 synchronous IOAS-copy privileged gate: PASS"
